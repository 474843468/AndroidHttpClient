...
public class SquareImageView extends ImageView {
  ...
  protected void onDraw(Canvas canvas) {
    ...
    int w = getWidth();//当前图层宽
    Bitmap roundBitmap = getCroppedBitmap(bitmap, w);
    canvas.drawBitmap(roundBitmap, 0, 0, null);

    Rect rec = canvas.getClipBounds();//得到裁剪的边界
    rec.top = rec.top + 2;
    ...
    paint.setShader(mShader);//  设置渐变色 这个正方形的颜色是改变的
    paint.setStyle(Paint.Style.STROKE);//填充样式
    paint.setStrokeWidth(5);//设置宽度
  ...
  public static Bitmap getCroppedBitmap(Bitmap bmp, int radius) {
    ...
    canvas.drawARGB(0, 0, 0, 0);// 1、将会以颜色ARBG填充整个控件的Canvas背景
    ...
    paint.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.SRC_IN));//图像混合模式
